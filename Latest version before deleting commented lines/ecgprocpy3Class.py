...
def set_ecg_thread_values(data_in, sr_in, run_flag_in, timeref, time_info):
    global gl_ecgdata, gl_sr, gl_run_flag, gl_timeref, gl_time_info
    gl_ecgdata = data_in
    gl_sr = sr_in
...
def set_son_values(sontype, amp, pan):
    ...
    ampValue = amp
    panning = pan
    # print('In set son values function')
    # print(sonType)
    # print(ampValue)
    # print(panning)
...
class ecgsig_proc_2(object):
    ...
    def r_peak_RT_det(self):
        ...
        while True:
            ...
            if gl_run_flag == True:
                ...
                gl_databuffer.extend(gl_ecgdata)
                current_time = gl_time_info

                if len(gl_databuffer) >= 8192 * 5:
                    # print 'Around a second of data is in the buffer'

                    self.find_R_peaks(gl_databuffer, window=20000, thpercentage=0.65, plot=True)
    ...
    def find_R_peaks(self, data, window=200, thpercentage=0.65, plot=False):
        ...
        thld = 15000  # This value should depend on the signal
...
class ecgsig_proc(object):
    """This is the class for processing ecg signals"""

    def __init__(self, ecgdata, samplerate):
        super(ecgsig_proc, self).__init__()
        self.data = ecgdata
        self.samplerate = samplerate
    ...
    def find_R_peaks(self, window=200, thpercentage=0.65, plot=False):
        '''This function filters the signal, calculates the time stamps for the peaks function and find
        the R peaks in one ECG lead'''
        sig = self.data
        # Filter the signal
        sigf = self.bpIIR_filter(fc1=10, fc2=50)
        # Hilbert transform
        ht = self.hilbert_transform(sigf)
        ...
        if plot == True:
            plt.clf()
            xax = np.linspace(0, len(sig) / self.samplerate, len(sig))
            th_arr = len(xax) * [thld]
...
# class ecg_features(object):
#     """docstring for ClassName"""
#
#     def __init__(self, ecgdata, peaks):
#         super(ecg_features, self).__init__()
#         self.data = ecgdata
#         self.peaks = peaks
#
#     def delta_t(self):
