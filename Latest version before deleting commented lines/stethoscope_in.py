import pyaudio
import numpy as np
# import stethoscope_processing as sp
import time
# import _thread
import threading
...
class receive_updated_GUI_values(object):
    def __init__(self, rate = 44100):
        # super(receive_updated_GUI_values, self).__init__()
    ...
    def receive_filter_slider_value(self, value):
        ...
        filter_cutoff = value
        # print (('a value received: %s') % (filter_cutoff))
...
class audio(object):
    'Class to input sound and process it'
    ...
    def __init__(self, channels=2, rate=44100, frames_per_buffer=2048, format = pyaudio.paInt16):
        ...
        # sr = rate # i should use only one of the rate variables
    ...
    def stop_stream(self):
        self.run_flag = False
        set_ecg_thread_values(self._ecg_data, self.audio_sr, self.run_flag, timeref, self.time_info['current_time']+timeref)
    ...
    def callback_pre_recorded(self, in_data, frame_count, time_info, status):
        ...
        '''This function shows how CardioScope works using pre-recorded data. 
        It doesn't create a new audio file. But it allows user to change test the GUI parameters in real-time. '''
    ...
    def callback(self, in_data, frame_count, time_info, status):
        ...
        # self.audio_amp_values_tunnel([np.random.random(16384)],[np.random.random(16384)]) -- To write anything on the plot
        # time.sleep(0.0001)
    ...
    def audio_amp_values_tunnel(self, leftChannel, rightChannel):
        "This function communicates with the GUI to plot acquired data"
